	...
	<view class="">
		...
		<view class="box">
			<view v-for="(item, index) in info.adminOrderVehicleInfoVoList" :key="index">
				...
				<view class="flexBox" style="border-bottom:2rpx dashed #999999;padding-bottom: 30rpx;">
					...
					<image v-if="carShow == false" style="width: 30rpx;height: 20rpx;" :src="$util.fileUrl('/xiangxia.png')" @click="getcarInfo" mode="aspectFill"></image>
					<image v-if="carShow == true" style="width: 30rpx;height: 20rpx;" :src="$util.fileUrl('/xiangshang.png')" @click="getcarInfo" mode="aspectFill"></image>
				</view>
			...
			<view class="flexBox" v-if="info.endDeliveryName == info.startDeliveryName">
				...
				<switch disabled="true" class="switch" :class="radio ? 'checked' : ''" :checked="radio ? true : false"></switch>
			</view>
		...
		<view class="box">
			<view class="flexBox">
				...
				<image v-if="peopleShow == false" style="width: 30rpx;height: 20rpx;" :src="$util.fileUrl('/xiangxia.png')" @click="getpeopleInfo" mode="aspectFill"></image>
				<image v-if="peopleShow == true" style="width: 30rpx;height: 20rpx;" :src="$util.fileUrl('/xiangshang.png')" @click="getpeopleInfo" mode="aspectFill"></image>
			</view>
			<view class="flexBox">
		...
		<view class="box">
			...
			<view class="flexBox" v-if="moneyShow == false">
				...
				<image
					v-if="moneyShow == false"
					style="width: 30rpx;height: 20rpx;margin-left: 10rpx;"
					:src="$util.fileUrl('/xiangxia.png')"
					@click="getmoneyInfo"
					mode="aspectFill"
				></image>
			</view>
			<view v-if="moneyShow == true" style="border-top:2rpx dashed #999999;margin-top: 20rpx;">
				...
				<view class="flexBox">
					...
					<image style="width: 30rpx;height: 20rpx;margin-left: 10rpx;" :src="$util.fileUrl('/xiangshang.png')" @click="getmoneyInfo" mode="aspectFill"></image>
				</view>
			</view>
		</view>
		...
		<view class="box">
			<view class="flexBox">
				...
				></image>
				<image
					v-if="invoiceShow == true"
					style="width: 30rpx;height: 20rpx;margin-left: 10rpx;"
					:src="$util.fileUrl('/xiangshang.png')"
					@click="getinvoiceInfo"
					mode="aspectFill"
				></image>
			</view>
		...
		<view class="flexBox" style="width: 90%;margin: 60rpx auto 90rpx auto;padding-bottom: 90rpx;">
			...
			<view style="width: 75%;display: flex;align-items: center;justify-content: flex-end;">
				...
				<view v-if="type == 5 && info.orderStatus == 5" class="lanbox" @click="jianyanshouche" :disabled="info.isPaymentIllegalDeposit == 1">检验收车</view>
				<view :disabled="info.isVehicleCertificates == true" v-if="type == 1" @click="deliverCar(1)" class="lanbox">交付车辆</view>
...
<script>
import { orderInfo, cancelOrder } from '@/apis/rentalOrder'
import { refundOfIllegalDeposit } from '@/apis/pay'
export default {
	data() {
		return {
...
switch .uni-switch-input::after {
	left: -4rpx;
	box-shadow: 0rpx 0rpx 10rpx rgba(0, 0, 0, 0.6);
}
