<template>
	<view class="home">
		<view class="flex-center tabList">
			<swiper class="swiperIcon" :indicator-dots="false" :indicator-color="indicatorColor" :indicator-active-color="indicatorActiveColor">
				<swiper-item class="flex flex-wrap swiperItem">
					<view v-for="(item, index) in swiperIcon" :key="index" class="flex-center flex-direction iconPanel" @click="toHomeLevel(item.url, item.id)">
		...
		<view class="flex-center flex-wrap panelList">
			<view class="panel" v-for="(item, index) in orderList" :key="index" @click="lookinfo(index)">
				...
				<view class="flex-center line"><i></i></view>
				<view class="content">
					<view class="flex titlePanel">
						<p class="name">{{ item.carModel }}</p>
						<view class="flex radioCheck">
							<p>异地还车</p>
							<switch disabled="true" @change.stop="radioChange" class="switch" :class="radio ? 'checked' : ''" :checked="radio ? true : false"></switch>
						</view>
					</view>
					...
					<view class="flex submit">
						...
						<view class="flex">
							<button
								:disabled="item.isCarTest == false"
								type="default"
								v-if="tabCheck == 0"
								class="flex-center btn"
								@tap.stop="toHomeLevel1('/pages/home/goInspect?obj=', item)"
							>
								出车检验
							</button>
							<button
								type="default"
								:disabled="item.isVehicleCertificates == true"
								v-if="tabCheck == 0"
								class="flex-center btn"
								style="margin-left: 20rpx;"
								@tap.stop="toHomeLevel2(item)"
							>
								交付车辆
							</button>
							<button type="default" v-if="tabCheck == 1" class="flex-center btn" @tap.stop="toHomeLevel1('/pages/home/deliverCar?type=2&obj=', item)">
								交车情况
							</button>
							<button
								type="default"
								v-if="tabCheck == 1 && item.orderStatus == 5"
								class="flex-center btn"
								style="margin-left: 20rpx;"
								@tap.stop="toHomeLevel1('/pages/home/inspectionCollect?obj=', item)"
								:disabled="item.isPaymentIllegalDeposit == 1"
							>
								检验收车
							</button>
							<button type="default" v-if="tabCheck == 2" class="flex-center btn" @tap.stop="toHomeLevel1('/pages/home/inspectionCollectInfo?obj=', item)">
								收车详情
							</button>
...
<script>
import { getOrderStatus, getOrderPageList } from '@/apis/rentalOrder'
import { authority } from '@/apis/admin'
