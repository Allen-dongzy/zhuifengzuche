...
/**
   ...
   */
function throttle(fn) {var wait = arguments.length > 1 && arguments[1] !== undefined ? arguments[1] : 1000;var timeStamp = arguments.length > 2 && arguments[2] !== undefined ? arguments[2] : false;
  var previous = 0;
  var timeout;
  return function () {
    var ctx = this;
...
function fileUrl(url) {
  var path = _config.default.filePath;
  return path + url;
}
...
/**
   ...
   */
function QR(url) {
  var path = _config.default.qrPath;
  return path + url;
}
...
function uploadImage(path) {
  ...
  return new Promise(function (resolve, reject) {
    uni.uploadFile({
      url: _config.default.uploadPath, //仅为示例，非真实的接口地址
      filePath: path,
      name: 'file',
      header: {
...
/**
   ...
   */function
awaitUpload(_x) {return _awaitUpload.apply(this, arguments);}
...
/**
                                                               * 时间日期转年月日格式
                                                               */function _awaitUpload() {_awaitUpload = _asyncToGenerator( /*#__PURE__*/_regenerator.default.mark(function _callee(fileList) {var i, obj;return _regenerator.default.wrap(function _callee$(_context) {while (1) {switch (_context.prev = _context.next) {case 0:i = 0;case 1:if (!(i < fileList.length)) {_context.next = 10;break;}_context.next = 4;return uploadImage(fileList[i]);case 4:obj = _context.sent;obj = JSON.parse(obj.data);fileList.splice(i, 1, obj.data);case 7:i++;_context.next = 1;break;case 10:return _context.abrupt("return", fileList);case 11:case "end":return _context.stop();}}}, _callee);}));return _awaitUpload.apply(this, arguments);}
...
function whileText(ctx, x, y, text) {
  ...
  for (var _i in textList) {
    ctx.fillText(textList[_i], x, y);
    y += 16; //列间距
  }
}
...

{
  ...
  checkPhone2: checkPhone2,
  fileUrl: fileUrl,
  QR: QR,
  awaitUpload: awaitUpload,
  getDateTime: getDateTime,
  getTimeToDate: getTimeToDate,
  roundRect: roundRect,
  roundArc: roundArc,
  whileText: whileText,
  toBack: toBack };exports.default = _default;
...
Object.defineProperty(exports, "__esModule", { value: true });exports.default = void 0;var _default = {
  path: 'http://192.168.0.233:5001/api', //服务器接口地址
  filePath: 'https://zdkj-oss-bucket.oss-cn-hangzhou.aliyuncs.com/zc-img', //静态资源地址
...
var open = function open(path) {var object = arguments.length > 1 && arguments[1] !== undefined ? arguments[1] : {};var closeSelf = arguments.length > 2 && arguments[2] !== undefined ? arguments[2] : 0;
  ...
  if (Object.prototype.toString.call(object) == '[object Number]') closeSelf = object;else
  if (object || Object.keys(object).length !== 0) {
    for (var key in object) {
      if (object.hasOwnProperty(key)) datas += "".concat(key, "=").concat(object[key], "&");
    }
  ...
  switch (parseInt(closeSelf)) {
    case 0:
      uni.navigateTo({
        url: !datas ? path : "".concat(path, "?").concat(datas) });
...
// 关闭页面
exports.open = open;var close = function close() {var num = arguments.length > 0 && arguments[0] !== undefined ? arguments[0] : 1;
  uni.navigateBack({
    delta: num });
...
// 弹出一个toast轻提示
exports.close = close;var toast = function toast(info) {var isMask = arguments.length > 1 && arguments[1] !== undefined ? arguments[1] : false;var position = arguments.length > 2 && arguments[2] !== undefined ? arguments[2] : 'bottom';
...
// 弹出一个toast图标提示
exports.toast = toast;var showToast = function showToast(info) {var iconType = arguments.length > 1 && arguments[1] !== undefined ? arguments[1] : 0;var isMask = arguments.length > 2 && arguments[2] !== undefined ? arguments[2] : true;var position = arguments.length > 3 && arguments[3] !== undefined ? arguments[3] : 'center';
  if (!info) return false;
  var icons = ['success', 'loading'];
  uni.showToast({
    icon: icons[iconType],
    title: info,
    mask: isMask,
...
// 弹出一个loading
exports.showToast = showToast;var showLoading = function showLoading() {var title = arguments.length > 0 && arguments[0] !== undefined ? arguments[0] : '';var isMask = arguments.length > 1 && arguments[1] !== undefined ? arguments[1] : true;
  uni.showLoading({
    title: title,
    mask: isMask });
...
// 弹出一个模态框
exports.showLoading = showLoading;var showModal = function showModal(_ref)





{var content = _ref.content,_ref$title = _ref.title,title = _ref$title === void 0 ? '提示' : _ref$title,_ref$showCancel = _ref.showCancel,showCancel = _ref$showCancel === void 0 ? true : _ref$showCancel,_ref$confirmText = _ref.confirmText,confirmText = _ref$confirmText === void 0 ? '确定' : _ref$confirmText,_ref$cancelText = _ref.cancelText,cancelText = _ref$cancelText === void 0 ? '取消' : _ref$cancelText;
  ...
  return new Promise(function (resolve, reject) {
    uni.showModal({
      ...
      success: function success(res) {
        if (res.confirm) resolve(0);else
        if (res.cancel) resolve(1);
      },
      fail: function fail(err) {
        reject(err);
      } });

  }).then(function (res) {return [null, res];}).catch(function (err) {return [err];});
};

// 选择图片-相机/相册
exports.showModal = showModal;var chooseImgs = function chooseImgs() {var count = arguments.length > 0 && arguments[0] !== undefined ? arguments[0] : 1;var type = arguments.length > 1 && arguments[1] !== undefined ? arguments[1] : 'all';var origin = arguments.length > 2 && arguments[2] !== undefined ? arguments[2] : false;
  var sourceType = ['album', 'camera'];
  if (type == 'album') sourceType = ['album'];else
  if (type == 'camera') sourceType = ['camera'];
  return new Promise(function (resolve, reject) {
    uni.chooseImage({
      count: count, // 相册下的可选图片个数
      sizeType: ['original', 'compressed'], // 可以指定是原图还是压缩图，默认二者都有
      sourceType: sourceType, // 从相册选择或相机拍摄
      success: function success(res) {
        resolve(res);
      },
      fail: function fail(err) {
    ...
    if (origin) return [null, res];else
...
exports.uploadFiles = uploadFiles;var previewImgs = function previewImgs(urls) {var current = arguments.length > 1 && arguments[1] !== undefined ? arguments[1] : 0;
  ...
  return new Promise(function (resolve, reject) {
    uni.previewImage({
      current: current, // 当前要显示图片的链接/索引值
      urls: urls,
      indicator: 'number',
      loop: false,
      success: function success(res) {
...
// 列表公共处理(可选配置)
exports.previewImgs = previewImgs;var listManager = function listManager(list, page, limit, config) {
  var dataStatus = 'more'; // 数据状态 more下拉前 loading下拉前 noMore下拉后-没有更多 noData下拉后-暂无数据
  var requestKey = true; // 请求开关
  var isRender = true; // 决定本次是否请求
  var listType = 'other'; // 判断列表类型(有时候是对象而非数组)
  if (Object.prototype.toString.apply(list) === '[object Object]') listType = 'object';else
  if (Object.prototype.toString.apply(list) === '[object Array]') listType = 'array';else
  return;
  // 获取列表长度
  var listLen = config && config.listLength || (listType == 'array' ? list.length : Object.keys(list).length);
...
// 获取app版本号
exports.listManager = listManager;var getVersion = function getVersion() {
...
// 清除缓存
exports.getVersion = getVersion;var clearCache = function clearCache() {
  var that = _this;
  var os = plus.os.name;
  uni.showLoading({
    title: '清除中' });
...
// 挂载uni小工具
exports.clearCache = clearCache;_vue.default.prototype.$open = open;
_vue.default.prototype.$close = close;
...
_vue.default.prototype.$showModal = showModal;
...
"use strict";
Object.defineProperty(exports, "__esModule", { value: true });exports.default = void 0;var _request = _interopRequireDefault(__webpack_require__(/*! ./request.js */ 572));function _interopRequireDefault(obj) {return obj && obj.__esModule ? obj : { default: obj };}var _default =
