class Handwriting {
    ...
    constructor(opts) {
        ...
        this.signaturePattern = opts.signaturePattern || 'landscape'//设置签名模式是横屏，还是竖屏 默认landscape（内置landscape:横屏；portrait：竖屏）
        this.canvasName = opts.canvasName || 'handWriting'
        this.canvasNameShadow = opts.canvasNameShadow || 'handWritingShadow'
    ...
    init() {
        ...
        console.log('this.signaturePattern==',this.signaturePattern);
        if(this.signaturePattern === 'landscape' ){
            console.log('进入landscape');
            this.context = uni.createCanvasContext(this.canvasNameShadow, this)
        }
    ...
    uploadScaleEnd(event) {
        ...
        if (this.currentLine.length > 2) {
            var info = (this.currentLine[0].time - this.currentLine[this.currentLine.length - 1].time) / this.currentLine
                .length;
        }
        ...
        return new Promise((resolve, rej) => {
            ...
            uni.canvasToTempFilePath({
                ...
                success: function(res) {
                    ...
                    if(that.signaturePattern === 'landscape' ){
                        ...
                        uni.getImageInfo({
                            ...
                            success: res => {
                                ...
                                let width = Math.round(res.height),
                                    height = Math.round(res.width);
                                that.context.rotate(-90 * Math.PI / 180);
                                that.context.drawImage(imgurl, -res.width, 0)
                        
                                that.context.draw(false, function(e) {
                        
                                    let time = setTimeout(() => {
                                        uni.canvasToTempFilePath({
                                            canvasId: that.canvasNameShadow,
                                            width: width,
                                            height: height,
                                            destWidth: width,
                                            destHeight: height,
