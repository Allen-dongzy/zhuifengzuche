<template>
	<view>
		<view style="width: 90%;margin: auto;padding-top: 130rpx;">
			<image style="height: 394rpx;width: 100%;position: relative;" :src="`${ossUrl}/mine/find-bg.png`"
				mode="aspectFill">
			</image>
		...
		<view style="display: flex;align-items: center;justify-content: center;margin-top: 6vh;">
			<view class="shareBox" @click="weChatShare('WXSceneSession')">
				<image style="height: 96rpx;width: 96rpx;" :src="`${ossUrl}/mine/wechat.png`" mode="aspectFill"></image>
				<view class="">微信好友</view>
			</view>
			<view class="shareBox" @click="weChatShare('WXSenceTimeline')">
				<image style="height: 96rpx;width: 96rpx;" :src="`${ossUrl}/mine/circle-of-friends.png`"
					mode="aspectFill">
				</image>
				<view class="">朋友圈</view>
			</view>
		</view>
		<view class="address">追风（重庆）汽车租赁有限公司</view>
...
<script>
	export default {
		...
		onLoad(e) {
			...
			uni.$emit('riskRefresh')
		},
		methods: {
			// 微信分享
			async weChatShare(scene) {
				const [err, res] = await uni.share({
					provider: 'weixin',
					scene,
					type: 5,
					summary: '查询您的风险系数！'
