<template>
	<view class="box">
		<view class="flexBox">
			...
			<view style="width: 56%;">
				<picker @change="platformChange" :value="platformIndex" :range="platformList" :range-key="'name'" class="pickerBox">
		...
		<view class="flexBox" v-if="orderKey">
			...
			<view style="width: 56%;">
				<picker @change="orderChange" :value="orderIndex" :range="orderList" :range-key="'show'" class="pickerBox">
		...
		<view class="flexBox" v-if="timeKey">
			...
			<view style="width: 66%;padding-left:100rpx">
				<picker mode="date" @change="dateChange" :value="timeDate" :start="startDate" :end="endDate" class="pickerBox">
					<view class="grayTetx">{{ timeDate || '请选择' }}</view>
				</picker>
			</view>
			<view style="width:4%;"><image style="width:28rpx;height: 28rpx;" :src="`${ossUrl}/mine/daytime.png`" mode="aspectFill"></image></view>
		</view>
		<view class="flexBox">
			<view class="blackTitle">项目</view>
			<view style="width: 56%;">
				<picker @change="projectChange" :value="projectIndex" :range="projectList" :range-key="'name'" class="pickerBox">
		...
		<view class="flexBox">
			...
			<view style="width: 59%;"><input class="grayTetx" type="text" v-model="carNumber" placeholder="请填写车牌号" /></view>
		</view>
		<view class="flexBox">
			<view class="blackTitle">店员</view>
			<view style="width: 59%;"><input class="grayTetx" type="text" v-model="staffName" placeholder="请填写店员" /></view>
		</view>
		<view class="flexBox">
			<view class="blackTitle">金额（元）</view>
			<view style="width: 59%;"><input class="grayTetx" type="digit" v-model="money" placeholder="请填写金额" /></view>
		</view>
		<view class="flexBox">
			<view class="blackTitle">{{ type === 0 ? '收' : '付' }}款人</view>
			<view style="width: 59%;"><input class="grayTetx" type="text" v-model="payer" placeholder="请填写付款人姓名" /></view>
		</view>
		<view class="flexBox" @click="$open('/pages/mine/bankCard', { mode: 'select' })" v-if="type === 0">
			...
			<view style="width: 56%;">
				<picker class="pickerBox" :disabled="true">
					<view class="grayTetx">
						{{
							Object.keys(bankCard).length > 0 ? `${bankCard.bankName} (${bankCard.name} 尾号${bankCard.cardNumber.slice(bankCard.cardNumber.length - 4)})` : '请选择'
						}}
					</view>
				</picker>
			</view>
		...
		<view style="background-color:#EFF0F3;margin-top: 20rpx;border-radius: 15rpx;width: 100%;padding: 20rpx;width: 93.5%;">
			...
			<view style="margin-top: 20rpx;">
				<view style="display: inline-block;width:120rpx;height: 120rpx;;margin-right:20rpx;position: relative;" v-for="(item, index) in imgList" :key="index">
					<view>
						<image style="width:120rpx;height: 120rpx" :src="item" mode="aspectFill" @click="previewPics(imgList, index)"></image>
						<image
							style="width:32rpx;height:32rpx;position: absolute;z-index:9;top:-10rpx;right: -16rpx;"
							:src="`${ossUrl}/common/lancha.png`"
							mode="aspectFill"
							@click.stop="removePic(index)"
						></image>
					</view>
				</view>
				<image v-if="imgList.length < 3" style="width:120rpx;height: 120rpx;" :src="`${ossUrl}/mine/beizhu.png`" mode="aspectFill" @click="choosePics"></image>
			</view>
		</view>
		<button
			style=" color: white;
			width: 100%;
				    ...
				    border-radius: 50px;
				    font-size: 32rpx;
				    height: 96rpx;line-height: 96rpx;margin-bottom:100rpx; "
			type="default"
			@click="receiptPaymentPayment"
		>
			{{ type === 0 ? '前往收款' : '前往付款' }}
		</button>
	</view>
...
export default {
	...
	methods: {
		...
		getCodeByWxCode: throttle(async function(info) {
			...
			// #endif
			// #ifdef MP-ALIPAY
			this.paymentPrecreate(res.data.user_id, info)
			// #endif
		}),
		// 发起支付
		async paymentPrecreate(payerUid, info) {
			const params = {
				reflect: info,
				orderId: info.id,
				payerUid,
