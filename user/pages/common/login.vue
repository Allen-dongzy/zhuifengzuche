<template>
	<view class="content">
		<view class="topimg"><image class="loginBox" :src="`${ossUrl}/common/logo.png`" mode="aspectFill"></image></view>
		<view class="topimg2"><image style="height:544rpx;width: 544rpx;" :src="`${ossUrl}/common/loginbg.png`" mode="aspectFill"></image></view>
		<view class="setbox">
			<view class="moreInpbox">
				<view style="width: 90%;"><input type="number" maxlength="11" class="inpBox" style="width: 95%;" v-model="phone" placeholder="请填写手机号" /></view>
			</view>
			<view class="line"></view>
			<view class="moreInpbox">
				<view style="width: 70%;"><input type="text" maxlength="6" class="inpBox" style="width: 95%;" v-model="code" placeholder="请填写验证码" /></view>
				<view class="btn" @click="sendLoginCode">{{ !codeKey ? `${countDown}s` : '获取验证码' }}</view>
			</view>
			<view class="line"></view>
			<view style="width: 86%;margin: auto;height: 96rpx;"><button style="color: #007AFF;border:1px solid #5A7EFF" @click="login">登录</button></view>
			<view class="selectBox">
				<view style="width: 32rpx;height: 32rpx;padding-top: 6rpx;" @click="lookAgreement">
					<image v-if="agreementType == false" style="height: 100%;width: 100%;" :src="`${ossUrl}/mine/quanxian2.png`" mode="aspectFill"></image>
					<image v-if="agreementType == true" style="height: 100%;width: 100%;" :src="`${ossUrl}/mine/quanxian1.png`" mode="aspectFill"></image>
				</view>
...
<script>
import { sendLoginCode, login } from '@/apis/sso'
