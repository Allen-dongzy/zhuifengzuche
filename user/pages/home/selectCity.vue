			...
			<view class="search">
				<view class="l-search">
					...
					<input
						...
						placeholder-class="default-style"
						placeholder-style="color:#8E8F97"
						:enableNative="false"
						:focus="searchFocus"
						@input="searchChange"
					/>
					<image class="clear-input" :src="`${ossUrl}/common/delete.png`" v-if="showClearBtn" @click="keyword = ''"></image>
				</view>
				<view class="r-cancel" open-type="navigateBack" delta="1" @click="searchMode = false">取消</view>
			</view>
			<!-- 搜索列表  -->
			<view class="reach-content" v-if="keyword">
				<block v-if="searchCityList.length">
					<view class="li" v-for="item in searchCityList" :key="item.id" @click="citySearchList(item)">{{ item.name }}</view>
				</block>
		...
		<view v-if="!searchMode">
			...
			<scroll-view class="scroll-view" scroll-y scroll-with-animation="true" enable-back-to-top="true" :scroll-into-view="toIndex" @scroll="scrollHandle">
				<view class="block">
					...
					<view class="area list-item" id="hot">
						...
						<view class="ul">
							<view class="li" v-for="(item, index) in hotCity" :key="index" @click="selectCity(item)">{{ item.name }}</view>
						</view>
					</view>
				</view>
				<!-- 城市列表  -->
				<view class="city-list">
					<view class="list list-item" v-for="(item, index) of allCity" :key="index" :id="index">
						...
						<view class="item" v-for="(inner, sub) in item" :key="inner.id" @click="selectCity(inner)">{{ inner.name }}</view>
					</view>
				</view>
			</scroll-view>
			...
			<view class="alphabet" @touchstart="touchStart" @touchend="touchEnd" @touchmove.stop="touchMove">
				<view v-for="(item, index) in alphabet" :key="index" @touchstart="getLetter" @touchend="setLetter" :id="item">
					...
					<view
						class="item"
						:class="{ active: currentLetter == item }"
			...
			this.timer = setTimeout(() => {
				...
				for (const i in this.allCity) {
					this.allCity[i].forEach(item => {
						if (item.name.includes(this.keyword) || item.shortName.includes(this.keyword)) {
							result.push(item)
						}
					})
				}
