	...
	<view class="store">
		<view class="address-title">
			...
			<view class="address" @click="openMap(-1)">
				<image class="icon-location" :src="`${ossUrl}/common/location-big.png`" mode="aspectFill"></image>
				<text>{{ address }}</text>
				...
				<view class="head">
					<view class="base-info">
						...
						<view class="title-box">
							...
							<view class="star-box">
								<image class="star" :src="`${ossUrl}/common/icon-star.png`" mode="aspectFill" v-for="(item, index) in starCode" :key="index"></image>
							</view>
						</view>
					</view>
					<view class="btn-group">
						<image class="btn" :src="`${ossUrl}/common/location-big.png`" mode="aspectFill" @click="openMap(index)"></image>
						<image class="btn" :src="`${ossUrl}/common/phone-big.png`" mode="aspectFill" @click="phoneCall(index)"></image>
					</view>
				</view>
				<view class="info">
					<view class="bar">
					...
					<view class="bar">
						...
						<view class="description">
							电话号码：
							<text @click="phoneCall(index)">{{ item.memberPhone }}</text>
						</view>
					</view>
					<view class="bar">
						<image class="icon" :src="`${ossUrl}/home/icon-message.png`" mode="aspectFill"></image>
						<view class="description" @click="$open('/pages/common/storeComment', { id: item.id })">评论：{{ item.evaluationNumber || 0 }}条</view>
					...
					<view class="bar">
						...
						<view class="description">
							门店地址：
							<text @click="openMap(index)">{{ item.memberAddress }}</text>
						</view>
					</view>
				</view>
			</view>
		...
		<view class="bottom"><view class="btn" @click="selAddress">选择该地点</view></view>
	</view>
</template>

<script>
import { deliveryFindShop } from '@/apis/delivery'
...
export default {
	...
	methods: {
		...
		async deliveryFindShop() {
			...
			const { requestKey, dataStatus, isRender } = listManager(res.data, this.page, this.size)
		...
		openMap(index) {
			...
			if (index >= 0) {
				...
				address = this.list[index].memberAddress
			} else {
