	...
	<view class="confirm-order">
		...
		<view class="info-card">
			...
			<view v-if="info.clientVehicleVo" class="params">
				{{ info.clientVehicleVo.gears }} / {{ info.clientVehicleVo.capacity }}座 / {{ info.clientVehicleVo.outputVolumeName }} / {{ info.clientVehicleVo.model }}
			</view>
		</view>
		<view class="lease">
		...
		<view class="function-box">
			<view class="item">
				...
				<view class="right" @click="selInsurance">
					<view class="text">
						￥
						<text>{{ info.orderPriceInfo.insuranceMoney }}</text>
					</view>
		...
		<view class="tab-box">
			...
			<view v-if="acIndex < 5" class="box box-voucher">
				...
				<view class="bottom-info">
					<image class="icon" :src="selStatus ? `${ossUrl}/order/sel-ac.png` : `${ossUrl}/order/sel.png`" mode="aspectFill" @click="tapIcon"></image>
					已阅读并同意
					<text @click="$open('/pages/common/carRentalAgreement')">《用户租车协议》</text>
				</view>
			</view>
				...
				<view class="list">
					<view class="item" v-for="(item, index) in invoiceList" :key="index" @click="selectInvoice(index)">
						<view class="name">
							...
							<view class="label" v-if="index == invoiceIndex" style="background-color: #FFA05B;">已选择</view>
						</view>
						<view class="number">
							{{ item.taxNum }}
							<view class="arrow"></view>
							<view class="click-block" @click.stop="goInvoiceInfo(item)"></view>
						</view>
					</view>
				</view>
				<view class="bottom-info">
		...
		<view class="bottom">
			...
			<view class="func-box">
				<view class="detail" @click="$open('/pages/order/costDetail', { info: JSON.stringify(info.orderPriceInfo) })">
					费用明细
					<view class="arrow"></view>
				</view>
				<!-- #ifdef MP-WEIXIN -->
...
import { rentalOrderGenerateOrder, rentalOrderCreateOrders } from '@/apis/rentalOrder'
import { findIsUseCouponByUser } from '@/apis/coupon'
import { invoiceQueryByUser } from '@/apis/invoice'
...
export default {
	...
	computed: {
		...
		totalPrice() {
			const price =
				(this.info.orderPriceInfo ? Number(this.info.orderPriceInfo.totalAndCoupon) : 0) -
		...
		bottomInfo() {
			...
			switch (this.acIndex) {
				...
				case 1:
					info =
						'下单半小时内免费取消，取车前12小时以上（不包含本数）取消全额退款，取车前12小时内取消订单将收取订单总额20%违约金，超过取车时间取消订单将收取订单总额30%违约金。'
					break
				case 2:
	...
	watch: {
		...
		totalPrice(newVal) {
			this.info.orderPriceInfo.total = newVal
		}
	},
	components: {
		EvanSwitch
	...
	.function-box {
		...
		.item {
			...
			.caption {
				...
				.label {
					@include box(124rpx, 34rpx, rgba(90, 126, 255, 0.1));
					border-radius: 4rpx;
					@include flex-center;
					@include font-set(16rpx, #5a7eff, 700);
					margin-left: 20rpx;
